...
namespace RRCGSource
{
    public static class GenerateChipsInterface
    {
        ...
        private static string ConvertToInterfaceDeclaration(string inputFile, string outputFile)
        {
            ...
            // Find method declarations with their comments, excluding deprecated methods
            var methodMatches = Regex.Matches(fileContent,
                @"(?:\/\/\/\s*<summary>\s*(?<comment>.*?)\s*<\/summary>)?\s*(?:public\s+|internal\s+)?static\s+(?!.*(?i)deprecated)(?<returnType>\w+)\s+(?<methodName>\w+)\s*\((?<parameters>.*?)\)")
                .Cast<Match>()
